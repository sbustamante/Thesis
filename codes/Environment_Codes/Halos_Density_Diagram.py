...
for fold in folds:
    ...
    number_halos_env = np.zeros((len(lambda_thr), 4))
    for i_lamb in xrange( len(lambda_thr) ):
	for i_hal in xrange(Nhalos):
	    for l in xrange(4):
		if enviroment( halos_envinroment[i_hal], lambda_thr[i_lamb] ) == l:
		    number_halos_env[i_lamb, l] += 1
		    
    #Plotting Environment
    plt.figure(figsize=(16,8.5))
    for l in xrange(4):
	#Density in each region
	plt.semilogy( lambda_thr, (number_halos_env[:,l]/volumen_environment[:,l])\
	/(np.sum(number_halos_env[0,:])/np.sum(volumen_environment[0,:])), 'o-', color = Colors[l], label = "%s"%env_labels[l] )

    plt.title('Density of isolated pairs respect to environment total volume')
    plt.ylabel('Density [%1.3f Number of Halos/Box Volume]'%(np.sum(number_halos_env[0,:])/np.sum(volumen_environment[0,:])))
    ...
    plt.savefig('Halos_Diagram_Density(%s_%d).pdf'%(labels[i_fold], res[i_fold]), format = 'pdf')

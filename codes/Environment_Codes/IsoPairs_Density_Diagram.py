...
for fold in folds:
    ...
    plt.ylabel('Density [%1.3e Number of Isolated Pairs/Box Volume]'%(np.sum(number_isop_env[0,:])/np.sum(volumen_environment[0,:])))
    ...
    plt.savefig('IsoPairs_Diagram_Density(%s_%d).pdf'%(labels[i_fold], res[i_fold]), format = 'pdf')

...
for fold in folds:
	...
	/(1.0*np.sum(number_isop_env[0,:])), 'o-', color = Colors[l], label = "Isopairs in%s"%env_labels[l] )
    ...
    plt.ylabel('Fraction of isolated pairs [%1.3e Number of LG Pairs]'%(np.sum(number_isop_env[0,:])))
    ...
    plt.savefig('IsoPairs_Number_Histogram(%s_%d)%s.pdf'%(labels[i_fold], res[i_fold], smooth), format = 'pdf')
